...
int audio_decode_frame(AVCodecContext *aCodecCtx, uint8_t *audio_buf, int buf_size) 
{
	...
	for(;;) {
		while(audio_pkt_size > 0) {
			...
			if (got_frame) {
				//buf_size = 8192;
				int len = swr_convert(audio_conv, 
						out, 
						buf_size/aCodecCtx->channels/av_get_bytes_per_sample(AV_SAMPLE_FMT_S16),  
						(const uint8_t **)frame->data ,
						frame->linesize[0]/aCodecCtx->channels/av_get_bytes_per_sample(frame->format));  
				if (len < 0) {
					perror("swr_convert");
					exit(1);
				}

				printf(" bufsize = %d  adfs = %d, sdfsdf  = %d\n", 
						buf_size, buf_size/aCodecCtx->channels/av_get_bytes_per_sample(AV_SAMPLE_FMT_S16),  
						frame->linesize[0]/aCodecCtx->channels/av_get_bytes_per_sample(frame->format));  
				printf("len = %d , frame = %d\n", len , frame->linesize[0]);
...
//					 frame.nb_samples,
				...
				//memcpy(audio_buf, frame.data[0], data_size);
				memcpy(audio_buf, frame->data[0], data_size);
				fprintf(stdout, "data_size = %d , buf_size=%d\n", data_size, buf_size);
			}
#endif
			if(data_size <= 0) {
...
{  
	...
	for(;!quit;){  
		...
		while(pkt_pos<pkt.size && !quit){  
			...
			if(frame_finished){  
				int len=swr_convert(audio_conv,out,buf_size/aCodecCtx->channels/av_get_bytes_per_sample(AV_SAMPLE_FMT_S16),  
...
int main(int argc, char *argv[]) 
{
	...
	//char *usr = "/root/1.flv";
	char *usr = "/root/yibuzhiyao.mp4";
	// Open video file
	if(avformat_open_input(&pFormatCtx, usr, NULL, NULL)!=0)
	...
	av_dump_format(pFormatCtx, 0, usr, 0);
	...
	wanted_channel_layout =   
		(aCodecCtx->channel_layout &&   
		 aCodecCtx->channels == av_get_channel_layout_nb_channels(aCodecCtx->channel_layout)) ? aCodecCtx->channel_layout : av_get_default_channel_layout(aCodecCtx->channels);  
	...
	aCodecCtx->channels=wanted_spec.channels;  
	aCodecCtx->channel_layout=wanted_channel_layout;  
	audio_conv=swr_alloc_set_opts(NULL, wanted_channel_layout,AV_SAMPLE_FMT_S16,aCodecCtx->sample_rate,  
			wanted_channel_layout,aCodecCtx->sample_fmt,  
